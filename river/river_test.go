...
func (s *riverTestSuite) SetUpSuite(c *C) {
	var err error
	s.c, err = client.Connect(*myAddr, "root", "MVPzhao@24", "mysqlsync")
	c.Assert(err, IsNil)
	...
	cfg.Sources = []SourceConfig{SourceConfig{Schema: "mysqlsync", Tables: []string{"test_river", "test_river_[0-9]{4}", "test_for_id", "test_for_json"}}}

	cfg.Rules = []*Rule{
		&Rule{Schema: "mysqlsync",
			Table:        "test_river",
			Index:        "river",
			Type:         "river",
